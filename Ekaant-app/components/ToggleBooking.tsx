...
export const ToggleBookingButton = () => {
  ...
  const bookingsForFriend = useSelector(
    (state) => state.user.bookingsForFriend
  );
  ...
  const toggleBooking = () => {
    ...
    if (friendDetails === null) {
      ...
      Toast.show("Please Select Friend Frist.", {
        type: "danger",
        duration: 2000,
      });
    ...
    if (
      !bookingForSelf &&
      (!friendDetails || Object.keys(friendDetails).length === 0)
    ) {
      Toast.show("Friend details are empty. Cannot toggle.", {
        type: "danger",
        duration: 2000,
      });
    ...
    dispatch(
      setFriendDetails({ friendDetails, bookingForSelf: !bookingForSelf })
    );
  };
  ...
  return (
    ...
    >
      <Text style={{ fontSize: w(12), fontFamily: ff.deckMedium }}>
        {bookingForSelf ? "Book for a Friend" : "Book for Myself"}
      </Text>
    </TouchableOpacity>
  );
};
