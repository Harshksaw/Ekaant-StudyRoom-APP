...
const AdminBookings = () => {
  ...
  const removeSeatBooking = async (seatId: string, selectedRoom: number) => {
    ...
    try {
      const response = await axios.post(
        `${BASEURL}/api/v1/admin/removeSeatBooking`,
        {
          libraryId: selectedLibrary,
          roomNo: selectedRoom,
          seatId,
      ...
      setSeats((prevSeats) =>
        prevSeats.map((seat) =>
          seat.id === seatId
            ? { ...seat, booked: false, bookedBy: null, bookingSource: null }
            : seat
        )
      );
      toast.success(response.data.message);
                ...
                <div key={row} className="flex mx-20">
                  {groupedSeats[Number(row)].map((seat, colIndex) => (
                    <div
                      ...
                      className={`mx-5 seat ${
                        seat.booked ? "booked" : ""
                      } bg-blue-300 flex flex-col justify-center items-center w-25 h-25 border border-gray-800 rounded-2xl p-4`}
                    >
                      <span>Seat {seat.label}</span>
                      {seat.booked ? (
                        <button
                          onClick={() =>
                            removeSeatBooking(seat.id, selectedRoom)
                          }
                        >
                          Remove Booking
                        </button>
                        ...
                        <button
                          onClick={() =>
                            bookSeat(seat.id, selectedRoom, seat.label)
                          }
                        >
                          Book Seat
                        </button>
