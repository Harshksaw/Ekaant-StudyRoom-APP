...
async function RegisterAdmin(req, res, next) {
  try {
    ...
    //     const token = jwt.sign({ admin_id: existingUser.id }, JWT_SECRET);
    //     return res.status(200).json({
    //       success: true,
    //       message: "Admin already registered",
    //       data: existingUser,
    ...
    //     return res.status(200).json({
    //       success: true,
    //       message: "Admin with this phone number is already registered",
    //       data: existingUser,
    //       token,
    //     });
    //   }
...
async function LoginAdmin(req, res) {
  ...
  try {
    ...
    const libraries = await prisma.library.findFirst({
      where: { libraryOwnerId: admin.id },
    });
    ...
    if (libraries.length > 1) {
      ...
      if (hasRooms.length === 0) {
        ...
        return res.status(200).json({
          success: true,
          message:
            "Admin authenticated successfully, but the library has  rooms.",
          hasRooms: true,
          data: admin,
          token,
        });
...
async function ResetAdminPassword(req, res, next) {
  ...
  const latest = await prisma.phoneOtp
...
async function BookSeat(req, res) {
  ...
  try {
    await prisma.$transaction(async (prisma) => {
      ...
      res
        .status(200)
        .json({ message: "Seat and timeslot booked successfully" });
    });
  } catch (error) {
  ...
  try {
    ...
    if (!seat.booked) {
      return res
        ...
        .json({ message: "Seat is not booked" });
    }
    ...
    res.status(StatusCodes.OK).json({
      message: "Seat booking removed successfully",
      data: updatedSeat,
    });
  } catch (error) {
    ...
    res
      .status(StatusCodes.INTERNAL_SERVER_ERROR)
      .json({ message: "Error removing seat booking", error: error.message });
  }
}
...
async function getEarnings(req, res) {
  ...
  try {
    const libraryId = await prisma.library.findFirst({
      where: { libraryOwnerId: adminId },
    });
    ...
    const totalEarnings = earnings.reduce(
      (total, booking) => total + booking.finalPrice,
      0
    );
    ...
    res
      .status(StatusCodes.INTERNAL_SERVER_ERROR)
      .json({ message: "Error getting earnings", error: error.message });
  }
}
